...
class CreateDataset:
    ...
    def __init__(self, base_dir, granularity, data_table=None, accel_table=None, gyro_table=None):
        self.base_dir = base_dir
        self.granularity = granularity
        self.data_table = data_table
        self.accel_table = accel_table
        self.gyro_table = gyro_table
        self.accel_data = []
        self.gyro_data = []
...
#                 self.data_table[str(prefix) + str(col)] = np.nan
        ...
        if aggregation == 'avg':
            ...
            self.dataset['activity'] = activity_type
            if sensor == 'accel':
                self.accel_data.append(self.dataset)
            else:
                self.gyro_data.append(self.dataset)
        else:
    ...
    def concat_datasets(self, sensor):
        if sensor == 'accel':
            self.accel_table = pd.concat(self.accel_data)
        else:
            self.gyro_table = pd.concat(self.gyro_data)
